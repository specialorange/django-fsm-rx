...
from django_fsm_rx.conf import FSMRXSettings, fsm_rx_settings
...
class TestAuditLogSettings:
    ...
    @pytest.mark.parametrize("audit_log,audit_log_mode,expected_enabled,expected_mode", [
    ...
    def test_audit_log_settings_combinations(
        self, audit_log, audit_log_mode, expected_enabled, expected_mode
    ):
        """Test various audit log settings combinations."""
        with override_settings(DJANGO_FSM_RX={
            'AUDIT_LOG': audit_log,
    ...
    @pytest.mark.parametrize("settings_dict,expected_atomic", [
        ({}, True),  # Default
        ({'ATOMIC': True}, True),
        ({'ATOMIC': False}, False),
...
class TestAuditModeExclusivity:
    ...
    def test_transaction_mode_does_not_trigger_signal_audit(self):
        ...
        with override_fsm_settings(AUDIT_LOG=True, AUDIT_LOG_MODE='transaction'):
            ...
            with patch('django_fsm_rx.audit.signal_audit_log') as mock_signal:
